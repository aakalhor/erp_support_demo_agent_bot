...
// local Ollama model (Qwen3 by default) to compose the answer. Two
// prompt modes:
//
//   - Grounded   — used when retrieval is confident. The LLM may only
//                  use the supplied FAQs.
//   - Conversational — used when retrieval is weak or empty. The LLM is
//                  free to greet, deflect off-topic asks, or politely
//                  defer, but still must never invent facts.
//
...
func NewOllamaAnswerGenerator(client *llm.Client) *OllamaAnswerGenerator {
	return &OllamaAnswerGenerator{client: client}
}

const groundedSystemPromptTemplate = `You are a friendly ERP support demo assistant. Answer the user's question using ONLY the information in the FAQs provided below. Follow these rules strictly:

1. Stay grounded. Do not introduce facts, numbers, prices, policies, phone numbers, emails, URLs, or contact details that are not present in the FAQs.
2. If the FAQs do not cover the question, reply naturally that you don't have a specific answer in the current demo knowledge base and that you can route the question to a support professional. Do not guess.
...
B) Specific ERP-related question for which the FAQs below provide no direct answer
   - Acknowledge the question in a natural way. Say you don't have a specific answer in the current demo knowledge base, and offer to route the question to a support professional if it's important.

C) Question clearly unrelated to ERP support (weather, jokes, math, news, etc.)
...
3. NEVER invent specific facts, numbers, prices, policies, phone numbers, emails, URLs, names, or product details that are not in the FAQs below.
...
func (g *OllamaAnswerGenerator) Generate(in port.GenerateInput) (string, error) {
	...
	if mode == modeGrounded {
		...
		user = buildGroundedUserPrompt(in)
	} else {
		sys = fmt.Sprintf(conversationalSystemPromptTemplate, target.HumanName())
		user = buildConversationalUserPrompt(in)
	}
...
func buildGroundedUserPrompt(in port.GenerateInput) string {
...
func buildConversationalUserPrompt(in port.GenerateInput) string {
	var b strings.Builder
	b.WriteString("User said: ")
	b.WriteString(strings.TrimSpace(in.Question))
	if len(in.Hits) > 0 {
		// Provide weak hits as soft context so the LLM can lightly draw
		// on them if obviously relevant, but the system prompt still
		// instructs it not to fabricate.
		b.WriteString("\n\nLoosely related material (may or may not be useful):\n")
		for i, h := range in.Hits {
			if i >= 3 {
				break
			}
